...
def generateMap2d(size_):
    ...
    yloc, xloc = [np.random.randint(
        0, size_x-1, 2), np.random.randint(0, size_y-1, 2)]
...
def plotMap(map2d_, path_, title_=''):
    ...
    #print(greennumber)
    #print(int(greennumber))
    colors = cm.winter(np.linspace(0, 1, int(greennumber)))
    ...
    colorsMap2d[locStart[0][0]][locStart[1][0]] = [.0, 1.0, .0, 1.0]  # lime
    colorsMap2d[locEnd[0][0]][locEnd[1][0]] = [.0, 1.0, 1.0, 1.0]  # cyan
    ...
    for iposExpand in range(len(locExpand[0])):
        colorsMap2d[locExpand[0][iposExpand]][locExpand[1][iposExpand]] = colors[int(map2d_[
            locExpand[0][iposExpand]][locExpand[1][iposExpand]])-1]
